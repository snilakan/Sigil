...
def main() :
	...
	#normalize all benchmarks
	for benchmark in benchmarks :
		factor = float(max_benchmark_swtime)/(benchmark.uppertree_softwaretime + benchmark.bottom_nodes_softwaretime)
		for node in benchmark.gran_table :
			node.scale(factor)
		benchmark.uppertree_softwaretime *= factor
		benchmark.bottom_nodes_softwaretime *= factor
	#del benchmarks[:] #Hopefully this only clears the references
	
	#select based on weightage of each benchmark. Amdahl's law sorting should take care of speedup sensitivity also
	#1. Keeping all weights equal, make a selection and print a table of the top few functions along with which domain they belong to.
	weights = {'graphics':0.2,'mining':0.2,'vision':0.2,'financial':0.2,'database':0.2} #In the order: 'graphics','mining','vision','financial','database' #'media' wasn't working when I was writing this
	apply_weights_and_select( weights, rootnode, max_benchmark_swtime )
	#List which applications are picked? We need some way of saying whats going on.
	print ""
	print "\nSort by (soft-comm/app.soft). For equal weightage"
	sortbyamdahl_and_print( gran_table, rootnode.time ) #Do a global ordering and print
	#Perhaps here we can have an experiment for area constrained selection - Todo later
	
	#Just let Amdahl's law thing sort it out. At the end of the day you are adding all execution times
	#2. Vary weights. Example: graphics and financial weighted more
	weights = {'graphics':0.35,'mining':0.2,'vision':0.25,'financial':0.1,'database':0.1} #Graphics heavy
	#weights = {'graphics':0.05,'mining':0.25,'vision':0.05,'financial':0.40,'database':0.25}	
	apply_weights_and_select( weights, rootnode, max_benchmark_swtime )
	#List which applications are picked? We need some way of saying whats going on.
	print ""
	print "\nSort by (soft-comm/app.soft). For unequal weightage"
	sortbyamdahl_and_print( gran_table, rootnode.time ) #Do a global ordering and print
	#Perhaps here we can have an experiment for area constrained selection - Todo later
