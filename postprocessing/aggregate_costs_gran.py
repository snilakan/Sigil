...
def main() :
	...
	if mode == 0 or mode == 2 :
		...
		if mode == 2 : #For granularity experiment
			...
			cg_anno = "perl valgrind-3.7/callgrind/callgrind_annotate --inclusive=yes --threshold=100 " + callgrind_filename
			...
			cg_anno = "perl valgrind-3.7/callgrind/callgrind_annotate --threshold=100 " + callgrind_filename
			temp = subprocess.Popen( cg_anno, shell=True, stdout=subprocess.PIPE )
			for l in temp.stdout :
				if re.match( r'.*:\w+', l ) :
