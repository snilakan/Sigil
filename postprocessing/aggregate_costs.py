...
overall_exec_cycles = 0
overall_cpu_cycles = 0
application_cpu_cycles = 0
application_exec_cycles = 0
gran_mode = 0

def usage() :
	print "Usage:"
	print "\t" + sys.argv[0] + " [<file>] [printcallees?=<yes|no>] [percentofinst=<0-100>] [mode=<modif|both>] [granmode=<nogran|coarsegran|+coverage|+progility|+speedup|+area>]"
...
def compare_callees_compcomm ( node, given_node ) :
	...
	for x in node.callees :
		...
		return_0_flag = compare_granularity_metric ( x, node ) #Will return 1 if metric is higher for x than node
		if return_0_flag :
			return 0
...
def search_for_l_in_cg ( line, funcinfotable, incl_flag ) :
	...
	* the bottom needs special processing
	'''
	global application_exec_cycles
		...
		if temp_name in line :
			...
			for node in funcnode.funcinsttable.itervalues() :
				...
				if float(diff)/min(line_instrs, node_instrs) < 0.01 :#if true, found!
					...
					if not incl_flag :
						application_exec_cycles += node.cg_params[13]
						application_cpu_cycles += node.cg_params[13]
					return
...
def calculate_metrics ( node ) :
	...
	#area = 467720 - 166.56 * ops/call+ 972.33 * totalmem/call + 0.011345 * ops/call^2 - 0.024587 * totalmem/call^2
	
	node.area = 467720 - 166.56 * float(node.iops_incl + node.flops_incl)/float(node.num_calls) + 972.33 * (float(node.ipcomm_incl_uniq + node.opcomm_incl_uniq + node.local_uniq)/float(node.num_calls)) + 0.011345 * (float(node.iops_incl + node.flops_incl)/float(node.num_calls))**2 - 0.024587 * (float(node.ipcomm_incl_uniq + node.opcomm_incl_uniq + node.local_uniq)/float(node.num_calls))**2
	...
	exec_cycles_cpu = node.cg_params[13]
	node.comp_cycles_accel = node.cg_params[0]/float(comp_speedup)
	if gran_mode == 0 :
		node.metric = node.progility
	elif gran_mode == 4 :
		...
		if (node.ipcomm_incl + node.local == 0) :
			node.metric = 0
			return
		input_offload_time = node.cg_params[14] * node.ipcomm_incl_uniq / float(node.ipcomm_incl + node.local) + node.ipcomm_incl_uniq/float(bandwidth)
		if (node.opcomm_incl + node.local == 0) :
			node.metric = 0
			return
		output_offload_time = node.cg_params[15] * node.opcomm_incl_uniq / float(node.opcomm_incl + node.local) + node.opcomm_incl_uniq/float(bandwidth)
		exec_cycles_accel = node.comp_cycles_accel + input_offload_time + output_offload_time #In CPU cycles
		overall_speedup	= exec_cycles_cpu/exec_cycles_accel #Should be in floating point
		node.metric = overall_speedup
		if gran_mode == 2 :
			node.metric *= node.coverage
		elif gran_mode == 3 :
			node.metric *= node.coverage * node.progility
		elif gran_mode == 5 :
			node.metric = node.metric * node.coverage * node.progility / node.area
	
	
def print_node( node, caller_star, append_flag ) :

	print "%50s %s %-10s %-15d %-15d %-15d %-10lu %-10lu %-10lu %-10lu %-10lu %-10lu %-15f %-15f %-15d %-25f %-15d %-15d" % (node.function_info.function_name, caller_star, node.num_calls, node.instrs_incl, node.flops_incl, node.iops_incl, node.ipcomm_incl_uniq, node.opcomm_incl_uniq, node.local_uniq, node.ipcomm_incl, node.opcomm_incl, node.local, node.comp_comm_uniq, node.comp_comm, node.cg_params[13], node.area, node.cg_params[14], node.cg_params[15])
...
def print_node_for_gran ( node, caller_star, append_flag ) :
	...
	print "%50s %s %-10s %-15d %-15d %-15d %-10lu %-10lu %-10lu %-10lu %-10lu %-10lu %-15f %-15f %-15d %-25f %-15d %-15d %-35.20f %-15f %-15f %-15f" % (node.function_info.function_name, caller_star, node.num_calls, node.instrs_incl, node.flops_incl, node.iops_incl, node.ipcomm_incl_uniq, node.opcomm_incl_uniq, node.local_uniq, node.ipcomm_incl, node.opcomm_incl, node.local, node.comp_comm_uniq, node.comp_comm, node.cg_params[13], node.area, node.cg_params[14], node.cg_params[15], node.metric, node.progility, node.coverage, node.comp_cycles_accel )
...
def final_calc_print_exectime ( bottom_list ) :
	...
	for node in bottom_list :
		if (node.ipcomm_incl + node.local == 0) :
			input_offload_time = 0
		else :
			input_offload_time = node.cg_params[14] * 100 * node.ipcomm_incl_uniq / float(node.ipcomm_incl + node.local) + node.ipcomm_incl_uniq/float(bandwidth)
		if (node.opcomm_incl + node.local == 0) :
			output_offload_time = 0
		else :
			output_offload_time = node.cg_params[15] * 100 * node.opcomm_incl_uniq / float(node.opcomm_incl + node.local) + node.opcomm_incl_uniq/float(bandwidth)
		exec_cycles_accel = node.comp_cycles_accel + input_offload_time + output_offload_time #In CPU cycles. For each node in the calltree, node.comp_cycles still has the inclusive accelerator cycles
		overall_exec_cycles += exec_cycles_accel 
		overall_cpu_cycles += node.cg_params[13] 
		application_exec_cycles += exec_cycles_accel #Even the accelerator time is in CPU cycles. All we have to do is multiply by the frequency
		application_cpu_cycles += node.cg_params[13] #Only for the bottom nodes does this param have an inclusive cost
	if not overall_exec_cycles :
		overall_speedup = 0
	else :
		overall_speedup = float(overall_cpu_cycles)/float(overall_exec_cycles)
	if not application_exec_cycles :
		application_speedup = 0
	else :
		application_speedup = float(application_cpu_cycles)/float(application_exec_cycles)
	print ""
	print "Bottom nodes Software Time (Cycles): %-20f" % (overall_cpu_cycles)
	print "Bottom nodes Hardware Time (Cycles): %-20f" % (overall_exec_cycles)
	print "Combined Speedup of bottom nodes: %-10f" % (overall_speedup)
	print "Overall Application Speedup: %-10f" % (application_speedup)
...
def main() :
	...
	# stick filename
	if len( sys.argv ) < 6 :  # no file name/no printcallees?/no percentage
	    # assume CLG_drwtest
		...
		if "nogran" in sys.argv[5] :
			...
			gran_mode = 4
		elif "coverage" in sys.argv[5] :
			gran_mode = 2
		elif "progility" in sys.argv[5] :
			gran_mode = 3
		elif "speedup" in sys.argv[5] :
			gran_mode = 1
		elif "area" in sys.argv[5] :
			gran_mode = 5
		else :
	...
	if mode == 0 or mode == 2 :
		...
		print "%-50s %s %-10s %-15s %-15s %-15s %-10s %-10s %-10s %-10s %-10s %-10s %-15s %-15s %-15s %-25s %-15s %-15s %-35s %-15s %-15s %-15s" % ("Function name", " ", "Numcalls", "Instrs", "Flops", "Iops", "Ipcomm_uq", "Opcomm_uq", "Local_uq", "Ipcomm", "Opcomm", "Local", "Comp_Comm_uniq", "Comp_Comm", "Est. Cyc", "Est. Area", "Est. Dlmr time", "Est. Dlmw time", "Der. metr", "progility", "coverage", "comp_cycles_acc")
		print ""
		...
		if mode == 2 :
			...
			cg_anno = "/archgroup/archtools/Profilers/valgrind-3.7_original/callgrind/callgrind_annotate --threshold=100 callgrind.out.orig.def"
			temp = subprocess.Popen( cg_anno, shell=True, stdout=subprocess.PIPE )
			for l in temp.stdout :
				if re.match( r'.*:\w+', l ) :
