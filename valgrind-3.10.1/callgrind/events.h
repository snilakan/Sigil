/*! \file
 *  An event type is somewhat arbitrarily implemented.  They somewhat
 *  match what is in Event.  An example of an event type is a "Dr" for
 *  a data read, or a "Drm1" for a level 1 cache miss on a data read.
 *  See the "see more" section for more details.  
 *
 *  EventGroups are made up of 0 or more event types.
 *  The groups are held in an array, with each group being a separate array element.
 *  An EventGroup is just a collection of related named events.
 *  As of Valgrind 3.10.1, these event groups are associated with IDs
 *  in global.h.  
 *
 *  EventSets are made up of 0 or more event groups.
 *  Different combinations of event groups (different event sets)
 *  are held in an array of event sets.
 *
 *  \see vgCallgrind_init_eventsets for implementation details
 */
...
struct _EventGroup {
    Int size; ///< How many event types make up the event group
    const HChar* name[0]; ///< The names of the event types
};
...
struct _EventSet {
    ...
    UInt mask; ///< Each bit corresponds to an EventGroup that has an ID matching the bit location
    Int count; ///< Number of event groups in the event set
    Int size; ///< Total number of event types in the event set
