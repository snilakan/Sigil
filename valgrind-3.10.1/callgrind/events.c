...
/*! \brief Initialize eventSetTable and eventGroup to NULL if first time calling function.
 */
static
...
/*! \brief Creates an EventSet based on a mask where
 *  each bit corresponds to an EventGroup.  
 * 
 *  Each bit in 'mask' corresponds to an EventGroup.
 *  It follows that if there are MAX_EVENTGROUP_COUNT EventGroups,
 *  then there would be 2^MAX_EVENTGROUP_COUNT possibilities.
 *  EventSets are held in eventSetTable, and there are accordingly
 *  2^MAX_EVENTGROUP_COUNT slots in this table (defined as MAX_EVENTSET_COUNT).
 *
 *  \see EventGroup
 *  \see EventSet
 */
...
/*! \returns EventSet* composed of a single EventGroup
 */
...
/*! \returns EventSet* composed of two EventGroups
 */
...
/*! Required for Sigil hack in sim.c
 *  \returns EventSet* composed of three EventGroups
 */
