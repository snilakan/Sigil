/*! \file
 *  \brief  Callgrind's main instrumentation and analyzation functions.
 *
 *  This file contains the bulk of Callgrind's frontend that is exposed 
 *  to Valgrind via callback functions passed to Valgrind.  Please read
 *  Valgrind's manual about tool requirements for more information.
 *  Basically there is a function that gets run before commandline arguments
 *  are processed, a function that gets run after commandline arguments are
 *  processed, a function that processes the VEX intermediate representation (IR)
 *  from Valgrind's core, and then a function to do anything else to wrap up.  
 *
 *  Note that *b*asic *b*locks (*BB*) and *s*uper *b*locks (*SB*), are used
 *  interchangeably at times, despite the fact that generally SBs are single
 *  entry, multiple exit blocks of instructions, while BBs are generally
 *  single entry, single exit.
 *
 *  \see vgCallgrind_pre_clo_init
 *  \see vgCallgrind_post_clo_init
 *  \see vgCallgrind_fini
 *  \see vgCallgrind_instrument
 *
 *  \todo Visually represent events structure
 */
...
{
   CLG_DEBUG(6, "log_ops: Optype => %d\n",Op);

   if (CLG_(current_state).collect) {
      ULong *cost_flops_iops;

      if (CLG_(current_state).nonskipped)
        cost_flops_iops = CLG_(current_state).nonskipped->skipped + fullOffset(EG_OPS);
      else
        cost_flops_iops = CLG_(cost_base) + ii->cost_offset + ii->eventset->offset[EG_OPS];

      if(Op <  Ity_F32)
        cost_flops_iops[1] ++;//the iops
      else if(Op < Ity_V128)
        cost_flops_iops[0] ++;//the flops
   }   
   ...
   /// \todo Merge multiple operations? Above comment is from Sid.  Mike to figure out what he meant.
   if(Op <  Ity_F32)
...
typedef
   enum {
      ...
      Ev_Op   // Floating-point or integer operation event
   }
   EventTag;
...
typedef
   struct {
      ...
      union {
         ...
         struct {
           IRType Op; 
         } Op; /// Sigil: added to count flops and iops
      } Ev;
   }
   Event;
   ...
   switch (ev->tag) {
	 ...
	 break;
      ...
      case Ev_G:
         VG_(printf)("G  %p\n", ev->inode);
         break;
      case Ev_Op: /// Sigil: added to count flops and iops
         VG_(printf)("Op  %p\n", ev->inode);
         break;
      default:
	 tl_assert(0);
...
/*! \brief Generate code for all outstanding memory events, and mark the queue
 *  empty.  Code is generated into cgs->sbOut, and this activity
 *  'consumes' slots in cgs->bb.
 *
 * 'Events' are generated in a global Callgrind structure and occasionally need
 * to be flushed...somewhere
 */
static void flushEvents ( ClgState* clgs )
{
   ...
   if (!clgs->seen_before) {
	   ...
	   case Ev_G:
	       ...
	       break;
           case Ev_Op: /// Sigil: added to count flops and iops
               // extend event set by Op counters 
               ev->inode->eventset = CLG_(add_event_group)(ev->inode->eventset,
                 EG_OPS);
   ...
   for(i = 0; i < clgs->events_used; i = inew) {
	 ...
	 case Ev_Dm:
	    ...
	    break;
         ...
         case Ev_Op: /// Sigil
            /* Flop/Int op event */
            helperName = "log_op_event";
            helperAddr = &log_ops_event;
            argv = mkIRExprVec_2( i_node_expr, mkIRExpr_HWord((HWord) ev->Ev.Op.Op) );
            regparms = 2;
            inew = i+1;
            break;
	 default:
 ...
 */
/*! \brief First pass over a BB to instrument, counting instructions and jumps
 *  This is needed for the size of the BB struct to allocate
 */
...
/*!  
 *  \brief Callgrind/Sigil instrumentation and analysis
 *
 *  Passed to the Valgrind core for instrumentation.
 *  In most cases, the VEX IR (from _sbIn_) is not actually instrumented.
 *  That is, the IR passed to Callgrind is the same IR passed back to Valgrind
 *  for execution.  However, Callgrind analyzes this IR and does additional
 *  bookkeeping to profile datapaths and callpaths within the guest program.
 *
 *  This is modified for Sigil: additional bookkeeping is done for datapath
...
/*!
 *  \brief Pre-commandline options function passed to Valgrind
 */
static
void CLG_(pre_clo_init)(void)
{
    ...
    VG_(track_new_mem_startup)( CLG_(new_mem_startup) );
    VG_(track_new_mem_mmap)   ( CLG_(new_mem_mmap)    );
    VG_(track_new_mem_brk)    ( CLG_(new_mem_brk)     );
